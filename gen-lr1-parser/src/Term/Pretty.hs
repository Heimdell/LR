...
import Text.PrettyPrint.HughesPJClass (text, Pretty(pPrint), parens, (<+>), doubleQuotes)
...
instance Pretty Term   where pPrint = doubleQuotes . pPrint . (.term)
instance Pretty Entity where pPrint = pPrint . (.entity)

instance Pretty Point where
  pPrint = \case
    T mbName term   -> maybe (pPrint term)   (\name -> parens (pPrint name <+> ":" <+> pPrint term)  ) mbName
    E mbName entity -> maybe (pPrint entity) (\name -> parens (pPrint name <+> ":" <+> pPrint entity)) mbName
